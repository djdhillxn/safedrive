...
\begin{tabular}{lll}
  ...
  Policy & MLP & MLP \\
  Action space & Continuous; steering $\pm0.1$ & Continuous; steering $\pm0.1$ \\
...
\begin{itemize}
  ...
  \item Seeds 2000--2099 were consumed by the second audit. Do not report them as held out;
        seeds 3000--3019 were consumed by the local PPO proof. Corrected frozen
        configurations use 4000--4019 once.
  \item Run the bounded SAC straight-road control in Colab and require its numerical gate.
  \item Evaluate the qualifying frozen checkpoints, then fill the report placeholders.
  \item Diagnose fixed-curve continuous steering only after the Phase-1 matrix is complete.
  \item Local macOS versus Colab runtime differences: TODO after both smoke tests.
\end{itemize}

\section{Deferred Phase-2 ideas}
Fixed-curve control, procedural maps, traffic, seed sweeps, richer observations, and
explicit safety interventions are intentionally outside the required Phase 1. Revisit them
in that order only after the straight-road IDM--PPO--SAC matrix has complete reproducible
artifacts and a finished report.
