...
% Phase-1 learned-policy values are known.  The IDM cells remain visibly pending
% until the exact held-out test is rerun and the fingerprint-checked comparison
% generates reports/generated_phase1_results.tex.
\newcommand{\PpoSuccess}{100.0\%}
\newcommand{\SacSuccess}{100.0\%}
\newcommand{\IdmSuccess}{Pending}
\newcommand{\PpoCollision}{0.0\%}
\newcommand{\SacCollision}{0.0\%}
\newcommand{\IdmCollision}{Pending}
\newcommand{\PpoOffRoad}{0.0\%}
\newcommand{\SacOffRoad}{0.0\%}
\newcommand{\IdmOffRoad}{Pending}
\newcommand{\PpoCompletion}{96.1\%}
\newcommand{\SacCompletion}{95.7\%}
\newcommand{\IdmCompletion}{Pending}
...
\begin{table}[h]
  \centering
  \caption{Held-out fixed-straight-road outcomes. The IDM row is populated only by the
  fingerprint-checked comparison generated from the exact 20-episode test.}
...
The latest IDM reproducibility gate ran validation seeds 1000--1009 twice. Both passes
produced identical per-scenario outcomes, 100\% success, 95.4\% route completion, and zero
failures. This verifies deterministic baseline execution. It is not substituted for the
held-out test. The comparison tool now refuses summaries whose map, traffic, horizon,
reward, action interface, or evaluation split is incompatible.
